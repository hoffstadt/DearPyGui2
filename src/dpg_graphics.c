...
#define DPG_MAX_FRAMES_IN_FLIGHT 2

//-----------------------------------------------------------------------------
// [SECTION] forward declarations
//-----------------------------------------------------------------------------

static VKAPI_ATTR VkBool32 VKAPI_CALL dpg__debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT tMsgSeverity, VkDebugUtilsMessageTypeFlagsEXT tMsgType, const VkDebugUtilsMessengerCallbackDataEXT* ptCallbackData, void* pUserData);

//-----------------------------------------------------------------------------
// [SECTION] internal functions
//-----------------------------------------------------------------------------

static uint32_t dpg__get_u32_max(uint32_t a, uint32_t b) { return a > b ? a : b;}
static uint32_t dpg__get_u32_min(uint32_t a, uint32_t b) { return a < b ? a : b;}

static int
dpg__select_physical_device(VkInstance tInstance, dpgVulkanDevice* ptDeviceOut)
{
    ...
    DPG_VULKAN(vkEnumeratePhysicalDevices(tInstance, &uDeviceCount, NULL));
    DPG_ASSERT(uDeviceCount > 0 && "failed to find GPUs with Vulkan support!");

    // check if device is suitable
    VkPhysicalDevice atDevices[16] = {0};
    DPG_VULKAN(vkEnumeratePhysicalDevices(tInstance, &uDeviceCount, atDevices));
    ...
    DPG_ASSERT(ptDeviceOut->tPhysicalDevice != VK_NULL_HANDLE && "failed to find a suitable GPU!");
...
void
dpg_create_instance(dpgVulkanGraphics* ptGraphics, uint32_t uVersion, bool bEnableValidation)
{
    static const char* pcKhronosValidationLayer = "VK_LAYER_KHRONOS_validation";

    const char** sbpcEnabledExtensions = NULL;
    dpg_sb_push(sbpcEnabledExtensions, VK_KHR_SURFACE_EXTENSION_NAME);

    #ifdef _WIN32
        dpg_sb_push(sbpcEnabledExtensions, VK_KHR_WIN32_SURFACE_EXTENSION_NAME);
    #elif defined(__APPLE__)
        dpg_sb_push(sbpcEnabledExtensions, "VK_EXT_metal_surface");
        dpg_sb_push(sbpcEnabledExtensions, VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
    #else // linux
        dpg_sb_push(sbpcEnabledExtensions, VK_KHR_XLIB_SURFACE_EXTENSION_NAME);
    #endif

    if(bEnableValidation)
        dpg_sb_push(sbpcEnabledExtensions, VK_EXT_DEBUG_UTILS_EXTENSION_NAME);

    dpg_create_instance_ex(ptGraphics, uVersion, bEnableValidation ? 1 : 0, &pcKhronosValidationLayer, dpg_sb_size(sbpcEnabledExtensions), sbpcEnabledExtensions);
}

void
dpg_create_instance_ex(dpgVulkanGraphics* ptGraphics, uint32_t uVersion, uint32_t uLayerCount, const char** ppcEnabledLayers, uint32_t uExtensioncount, const char** ppcEnabledExtensions)
{
    ...
    DPG_VULKAN(vkEnumerateInstanceLayerProperties(&uInstanceLayersFound, NULL));
    if(uInstanceLayersFound > 0)
    {
        ptAvailableLayers = (VkLayerProperties*)malloc(sizeof(VkLayerProperties) * uInstanceLayersFound);
        DPG_VULKAN(vkEnumerateInstanceLayerProperties(&uInstanceLayersFound, ptAvailableLayers));
    }

    // retrieve supported extensions
    uint32_t uInstanceExtensionsFound = 0u;
    VkExtensionProperties* ptAvailableExtensions = NULL;
    DPG_VULKAN(vkEnumerateInstanceExtensionProperties(NULL, &uInstanceExtensionsFound, NULL));
    if(uInstanceExtensionsFound > 0)
    {
        ...
        DPG_VULKAN(vkEnumerateInstanceExtensionProperties(NULL, &uInstanceExtensionsFound, ptAvailableExtensions));
    }
    ...
    for(uint32_t i = 0; i < uExtensioncount; i++)
    {
        ...
        if(!extensionFound)
        {
            dpg_sb_push(sbpcMissingExtensions, requestedExtension);
        }
    }

    // report if all requested extensions aren't found
    if(dpg_sb_size(sbpcMissingExtensions) > 0)
    {
        printf("%d %s\n", dpg_sb_size(sbpcMissingExtensions), "Missing Extensions:");
        for(uint32_t i = 0; i < dpg_sb_size(sbpcMissingExtensions); i++)
        {
            printf("  * %s\n", sbpcMissingExtensions[i]);
        }
        DPG_ASSERT(false && "Can't find all requested extensions");
    }
    ...
    for(uint32_t i = 0; i < uLayerCount; i++)
    {
        ...
        if(!bLayerFound)
        {
            dpg_sb_push(sbpcMissingLayers, pcRequestedLayer);
        }
    }

    // report if all requested layers aren't found
    if(dpg_sb_size(sbpcMissingLayers) > 0)
    {
        printf("%d %s\n", dpg_sb_size(sbpcMissingLayers), "Missing Layers:");
        for(uint32_t i = 0; i < dpg_sb_size(sbpcMissingLayers); i++)
        {
            printf("  * %s\n", sbpcMissingLayers[i]);
        }
        DPG_ASSERT(false && "Can't find all requested layers");
    }
    ...
    VkDebugUtilsMessengerCreateInfoEXT tDebugCreateInfo = {
        ...
        .pfnUserCallback = dpg__debug_callback,
    ...
    DPG_VULKAN(vkCreateInstance(&tCreateInfo, NULL, &ptGraphics->tInstance));
    printf("%s\n", "created Vulkan tInstance.");

    // cleanup
    if(ptAvailableLayers)     free(ptAvailableLayers);
    if(ptAvailableExtensions) free(ptAvailableExtensions);
    dpg_sb_free(sbpcMissingLayers);
    dpg_sb_free(sbpcMissingExtensions);
    ...
    if(bValidationEnabled)
    {
        ...
        DPG_ASSERT(func != NULL && "failed to set up debug messenger!");
        DPG_VULKAN(func(ptGraphics->tInstance, &tDebugCreateInfo, NULL, &ptGraphics->tDbgMessenger));
        printf("%s\n", "enabled Vulkan validation layers.");       
    }
}

void
dpg_create_device(VkInstance tInstance, VkSurfaceKHR tSurface, dpgVulkanDevice* ptDeviceOut, bool bEnableValidation)
{
    ...
    iDeviceIndex = dpg__select_physical_device(tInstance, ptDeviceOut);
    ...
    for(uint32_t i = 0; i < uQueueFamCnt; i++)
    {
        ...
        DPG_VULKAN(vkGetPhysicalDeviceSurfaceSupportKHR(ptDeviceOut->tPhysicalDevice, i, tSurface, &tPresentSupport));
    ...
    DPG_VULKAN(vkCreateDevice(ptDeviceOut->tPhysicalDevice, &tCreateDeviceInfo, NULL, &ptDeviceOut->tLogicalDevice));
...
void
dpg_create_frame_resources(dpgVulkanGraphics* ptGraphics, dpgVulkanDevice* ptDevice)
{
    // create command pool
    ...
    DPG_VULKAN(vkCreateCommandPool(ptDevice->tLogicalDevice, &tCommandPoolInfo, NULL, &ptGraphics->tCmdPool));
    ...
    ptGraphics->uFramesInFlight = DPG_MAX_FRAMES_IN_FLIGHT;
    dpg_sb_reserve(ptGraphics->sbFrames, ptGraphics->uFramesInFlight);
    for(uint32_t i = 0; i < ptGraphics->uFramesInFlight; i++)
    {
        dpgVulkanFrameContext tFrame = {0};
        DPG_VULKAN(vkCreateSemaphore(ptDevice->tLogicalDevice, &tSemaphoreInfo, NULL, &tFrame.tImageAvailable));
        DPG_VULKAN(vkCreateSemaphore(ptDevice->tLogicalDevice, &tSemaphoreInfo, NULL, &tFrame.tRenderFinish));
        DPG_VULKAN(vkCreateFence(ptDevice->tLogicalDevice, &tFenceInfo, NULL, &tFrame.tInFlight));
        DPG_VULKAN(vkAllocateCommandBuffers(ptDevice->tLogicalDevice, &tAllocInfo, &tFrame.tCmdBuf));  
        dpg_sb_push(ptGraphics->sbFrames, tFrame);
    }
}

void
dpg_create_framebuffers(dpgVulkanDevice* ptDevice, VkRenderPass tRenderPass, dpgVulkanSwapchain* ptSwapchain)
{
    for(uint32_t i = 0; i < ptSwapchain->uImageCount; i++)
    {
        ...
        DPG_VULKAN(vkCreateFramebuffer(ptDevice->tLogicalDevice, &tFrameBufferInfo, NULL, &ptSwapchain->ptFrameBuffers[i]));
    }
}

void
dpg_create_swapchain(dpgVulkanDevice* ptDevice, VkSurfaceKHR tSurface, uint32_t uWidth, uint32_t uHeight, dpgVulkanSwapchain* ptSwapchainOut)
{
    ...
    DPG_VULKAN(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(ptDevice->tPhysicalDevice, tSurface, &tCapabilities));

    uint32_t uFormatCount = 0u;
    DPG_VULKAN(vkGetPhysicalDeviceSurfaceFormatsKHR(ptDevice->tPhysicalDevice, tSurface, &uFormatCount, NULL));
    ...
    VkBool32 tPresentSupport = false;
    DPG_VULKAN(vkGetPhysicalDeviceSurfaceSupportKHR(ptDevice->tPhysicalDevice, 0, tSurface, &tPresentSupport));
    DPG_ASSERT(uFormatCount > 0);
    DPG_VULKAN(vkGetPhysicalDeviceSurfaceFormatsKHR(ptDevice->tPhysicalDevice, tSurface, &uFormatCount, ptSwapchainOut->ptSurfaceFormats_));

    uint32_t uPresentModeCount = 0u;
    DPG_VULKAN(vkGetPhysicalDeviceSurfacePresentModesKHR(ptDevice->tPhysicalDevice, tSurface, &uPresentModeCount, NULL));
    DPG_ASSERT(uPresentModeCount > 0 && uPresentModeCount < 16);

    VkPresentModeKHR atPresentModes[16] = {0};
    DPG_VULKAN(vkGetPhysicalDeviceSurfacePresentModesKHR(ptDevice->tPhysicalDevice, tSurface, &uPresentModeCount, atPresentModes));
    ...
    DPG_ASSERT(bPreferenceFound && "no preferred tSurface format found");
    ...
    {
        tExtent.width = dpg__get_u32_max(tCapabilities.minImageExtent.width, dpg__get_u32_min(tCapabilities.maxImageExtent.width, uWidth));
        tExtent.height = dpg__get_u32_max(tCapabilities.minImageExtent.height, dpg__get_u32_min(tCapabilities.maxImageExtent.height, uHeight));
    }
    ...
    DPG_VULKAN(vkCreateSwapchainKHR(ptDevice->tLogicalDevice, &tCreateSwapchainInfo, NULL, &ptSwapchainOut->tSwapChain));
    ...
    for(uint32_t i = 0; i < ptSwapchainOut->uImageCount; i++)
    {
        ...
        DPG_VULKAN(vkCreateImageView(ptDevice->tLogicalDevice, &tViewInfo, NULL, &ptSwapchainOut->ptImageViews[i]));   
    }  //-V1020
}

void
dpg_cleanup_graphics(dpgVulkanGraphics* ptGraphics, dpgVulkanDevice* ptDevice)
{
...
dpg_get_frame_resources(dpgVulkanGraphics* ptGraphics)
{
    return &ptGraphics->sbFrames[ptGraphics->szCurrentFrameIndex];
}

uint32_t
dpg_find_memory_type(VkPhysicalDeviceMemoryProperties tMemProps, uint32_t uTypeFilter, VkMemoryPropertyFlags tProperties)
{
...
void 
dpg_transition_image_layout(VkCommandBuffer tCommandBuffer, VkImage tImage, VkImageLayout tOldLayout, VkImageLayout tNewLayout, VkImageSubresourceRange tSubresourceRange, VkPipelineStageFlags tSrcStageMask, VkPipelineStageFlags tDstStageMask)
{
...
dpg__debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT tMsgSeverity, VkDebugUtilsMessageTypeFlagsEXT tMsgType, const VkDebugUtilsMessengerCallbackDataEXT* ptCallbackData, void* pUserData) 
{
